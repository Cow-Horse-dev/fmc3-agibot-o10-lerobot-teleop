...
class AirbotTOK2Follower(Robot):
    config_class = AirbotTOK2FollowerConfig
    name = "airbot_TOK2_follower"

    def __init__(self, config: AirbotTOK2FollowerConfig):
    ...
    def servo_left_joint_pos(self, joints: typing.List[float], eef, vel=3, eff=8.0):
        velocities = [vel] * (self.config.arm_joints_num - 1) + [30]
        effort = [eff] * (self.config.arm_joints_num - 1) + [4.0]
        pos = joints + [eef]
        return self.left_arm.pvt(pos, velocities, effort)

    def servo_right_joint_pos(self, joints: typing.List[float], eef, vel=3, eff=8.0):
        velocities = [vel] * (self.config.arm_joints_num - 1) + [30]
        effort = [eff] * (self.config.arm_joints_num - 1) + [4.0]
        pos = joints + [eef]

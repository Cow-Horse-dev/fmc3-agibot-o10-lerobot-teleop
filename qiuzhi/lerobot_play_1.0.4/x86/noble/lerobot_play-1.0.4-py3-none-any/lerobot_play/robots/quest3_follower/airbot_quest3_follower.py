...
class Quest3Follower(Robot):
    config_class = Quest3FollowerConfig
    name = "quest3_follower"

    def __init__(self, config: Quest3FollowerConfig):
    ...
    def servo_left_joint_pos(self, joints: typing.List[float], eef, vel=6.0, eff=8.0):
    ...
    def servo_right_joint_pos(self, joints: typing.List[float], eef, vel=6.0, eff=8.0):
        velocities = [vel] * (self.config.arm_joints_num - 1) + [25]
        effort = [eff] * (self.config.arm_joints_num - 1) + [5.0]
        pos = joints + [eef]

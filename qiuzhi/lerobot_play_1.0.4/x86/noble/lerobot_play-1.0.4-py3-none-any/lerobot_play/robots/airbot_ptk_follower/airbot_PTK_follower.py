...
class AirbotPTKFollower(Robot):
    config_class = AirbotPTKFollowerConfig
    name = "airbot_PTK_follower"

    def __init__(self, config: AirbotPTKFollowerConfig):
    ...
    def servo_left_joint_pos(self, joints: typing.List[float], eef, vel=1.0, eff=10.0):
        velocities = [vel] * self.config.arm_joints_num
        effort = [eff] * self.config.arm_joints_num
        pos = joints + [eef]
        return self.left_arm.pvt(pos, velocities, effort)

    def servo_right_joint_pos(self, joints: typing.List[float], eef, vel=1.0, eff=10.0):
        velocities = [vel] * self.config.arm_joints_num
        effort = [eff] * self.config.arm_joints_num
        pos = joints + [eef]
    ...
    def reset_init(self):
        ...
        pos = [
            [-0.06, -1.12, 0.92, 1.36, -1.02, -1.34, 0.0],
            [0.06, -1.12, 0.92, -1.36, 1.02, 1.34, 0.0],
        ]
        ...
        while True:
            ...
            if left_arm_arrived and right_arm_arrived:
                ...
                self.left_arm.pvt(pos[0], velocities, effort)
                self.right_arm.pvt(pos[1], velocities, effort)
            time.sleep(0.004)
            # print(velocities)

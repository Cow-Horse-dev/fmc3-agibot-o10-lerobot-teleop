...
def write_image_to_mcap(
    image: np.ndarray | PIL.Image.Image,
    fpath: Path,
    topic_name: str = "image",
    frame_index: int = 0,
    quality: int = 95,  # JPEG质量 1-100，85是较好的平衡点
    optimize: bool = False,
):
    """使用JPEG编码保存图像，文件更小，速度更快"""
    try:
        # 格式转换
        if isinstance(image, np.ndarray):
            img = image_array_to_pil_image(image)
        ...
        img.save(img_byte_arr, format="JPEG", quality=quality, optimize=optimize)
        ...
        with fpath.open("wb") as f:
            ...
            channel_id = writer.register_channel(
                topic=topic_name, message_encoding="image/jpeg", schema_id=0  # 改为jpeg
            )
        ...
        print(f"Error writing image to MCAP {fpath}: {e}")
...
def worker_thread_loop(queue: queue.Queue):
    while True:
        ...
        write_image_to_mcap(image_array, fpath)
        queue.task_done()

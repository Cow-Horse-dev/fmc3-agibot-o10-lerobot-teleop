...
DEFAULT_MCAP_PATH = (
    "mcap_temp/{image_key}/episode-{episode_index:06d}/frame-{frame_index:06d}.mcap"
)
DEFAULT_MCAP_SAVE_PATH = "mcap/{image_key}/episode-{episode_index:06d}/{image_key}.mcap"
...
def _encode_video_worker(
    ...
    # Prefer H.264 for recorded dataset videos so the resulting mp4 files open
    # reliably in common desktop players and browsers.
    encode_video_frames(img_dir, temp_path, fps, vcodec="h264", overwrite=True)
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _get_mcap_file_path(
        self, episode_index: int, image_key: str, frame_index: int
    ) -> Path:
        fpath = DEFAULT_MCAP_PATH.format(
            image_key=image_key, episode_index=episode_index, frame_index=frame_index
        )
        return self.root / fpath
    ...
    def _save_mcap(
        ...
        if self.image_writer is None:
            if isinstance(image, torch.Tensor):
                image = image.cpu().numpy()
            write_mcap(image, fpath, frame_index=frame_index)
        else:
            self.image_writer.save_mcap(image=image, fpath=fpath)

    def add_frame(
        self, frame: dict, use_mcap: bool, online_encoding: bool = False
    ) -> None:
        """
        ...
        for key in frame:
            ...
            if self.features[key]["dtype"] in ["image", "video"]:
                ...
                if not use_mcap:
                    ...
                    if online_encoding:
                        # 内存模式：直接流式编码为 H.265 MCAP
                        # 不再缓冲图像，解决内存占用和相机驱动阻塞问题
                        self._stream_encode_frame(key, frame[key], frame_index)
                        # Placeholder for episode_buffer to maintain length consistency
                        # self.episode_buffer[key].append(None)
                    else:
                        # 文件模式：先保存为 PNG，再在 save_episode 时批量编码为 H.265 MCAP
                        img_path = self._get_mcap_file_path(
                            episode_index=self.episode_buffer["episode_index"],
                            image_key=key,
                            frame_index=frame_index,
                        )
                        if frame_index == 0:
                            img_path.parent.mkdir(parents=True, exist_ok=True)
                        self._save_mcap(frame[key], img_path, frame_index)
                        self.episode_buffer[key].append(str(img_path))
    ...
    def save_episode(
        ...
        if use_mcap:
            ...
            ep_stats = compute_episode_stats(
                episode_buffer, self.features, use_mcap=use_mcap
            )
                ...
                if self.online_encoding:
                    ...
                    for key in self.features:
                        if self.features[key]["dtype"] in ["image", "video"]:
                            ...
                            output_path = DEFAULT_MCAP_SAVE_PATH.format(
                                image_key=key, episode_index=episode_index
                            )
                            # print(mcap_path.parent)
                            # print(self.root / output_path)

                            self.save_episode_mcap_async(
                                mcap_path.parent, self.root / output_path
                            )
    ...
    def save_episode_mcap_async(self, input_dir, output_file):
        """异步合并MCAP文件 - 简单版本"""

        p = mp.Process(
            target=self._save_episode_mcap,
        ...
        # 只记录PID，不保存进程对象
        pid = p.pid
        task_name = f"合并_{Path(input_dir).name}"
        self._async_pids.add(pid)

        print(f"启动进程 {pid}: {task_name}")
    ...
    def wait_all_async_tasks(self):
        """专为您的MCAP合并任务设计的等待方案"""
        import time
        import psutil

        print(f"📊 开始等待 {len(self._async_pids)} 个MCAP合并任务...")

        start_time = time.time()
        check_count = 0

        while self._async_pids:
            check_count += 1
            completed_pids = []

            for pid in list(self._async_pids):
                try:
                    ...
                    if status == psutil.STATUS_ZOMBIE:
                        ...
                        # 🔄 进程还在运行，检查运行时间
                        if check_count % 10 == 0:  # 每10次检查打印一次
                            runtime = time.time() - proc.create_time()
                            cpu = proc.cpu_percent()
                            memory_mb = proc.memory_info().rss / 1024 / 1024

                            print(f"🔄 进程 {pid} 仍在合并数据:")
                        ...
                        # 其他状态（睡眠、磁盘休眠等）
                        print(f"ℹ️  进程 {pid} 状态: {status}")

                except psutil.NoSuchProcess:
                    # ✅ 进程已完全结束
                    print(f"✅ 进程 {pid} 已完全结束")
                    completed_pids.append(pid)

                except Exception as e:
                    print(f"⚠️  检查进程 {pid} 时出错: {e}")

            # 移除已完成的进程
            for pid in completed_pids:
                self._async_pids.remove(pid)

            # 显示等待进度
            if self._async_pids:
                elapsed = time.time() - start_time
                remaining = len(self._async_pids)
                print(f"\n⏳ 等待进度: 已等待 {elapsed:.1f}秒，还有 {remaining} 个任务")
                print("正在等待任务完成，请勿中断...")
                time.sleep(2)  # 2秒检查一次

        total_time = time.time() - start_time
        print(f"\n🎉 所有 {check_count} 个MCAP合并任务已完成！")
        print(f"   总等待时间: {total_time:.1f}秒")
        print("✅ 数据处理完成，可以安全退出程序")

    def _stream_encode_frame(
        self, key: str, frame: np.ndarray, frame_index: int
    ) -> None:
        """Stream encode a single frame to H.265 MCAP."""
        import av
        import fractions
        from mcap.writer import Writer
        ...
        if key not in self.streaming_encoders:
            ...
            output_path = mcap_dir / f"{key}.mcap"
            ...
            codec = av.CodecContext.create("hevc_nvenc", "w")
            codec.width = width
            codec.height = height
            codec.pix_fmt = "yuv420p"
            codec.gop_size = self.fps * 4
            ...
            try:
                ...
                print(f"[{key}] NVENC init failed: {e}. Fallback to libx265.")
                codec = av.CodecContext.create("libx265", "w")
                codec.width = width
                codec.height = height
                codec.pix_fmt = "yuv420p"
                codec.time_base = fractions.Fraction(1, self.fps)
                codec.options = {"crf": "28", "preset": "ultrafast"}
                codec.open()

            self.streaming_encoders[key] = {
                "file": f,
                "writer": writer,
                "codec": codec,
            ...
            print(f"[{key}] Stream encoding initialized -> {output_path.name}")

        encoder = self.streaming_encoders[key]
        codec = encoder["codec"]
        writer = encoder["writer"]
        channel_id = encoder["channel_id"]
        ...
        video_frame = av.VideoFrame.from_ndarray(frame_hwc, format="rgb24")
        video_frame = video_frame.reformat(format="yuv420p")
        video_frame.pts = frame_index
        video_frame.time_base = codec.time_base
        ...
        for packet in packets:
            ts_ns = encoder["start_time"] + int(
                frame_index * (1_000_000_000 / self.fps)
            )
    ...
    def _close_stream_encoders(self) -> None:
        """Flush and close all active streaming encoders."""
        if not self.streaming_encoders:
            return

        print("Finalizing H.265 streams...")
        for key, encoder in self.streaming_encoders.items():
            codec = encoder["codec"]
            writer = encoder["writer"]
            channel_id = encoder["channel_id"]
            ...
            try:
                ...
                print(f"[{key}] Error flushing codec: {e}")

            writer.finish()
            encoder["file"].close()
            print(f"[{key}] Stream closed.")

        self.streaming_encoders = {}

    def _encode_images_to_mcap_memory(
        self, images: list[np.ndarray], output_path: Path
    ):
        """将内存中的图像列表直接编码为 H.265 MCAP 文件"""
        from mcap.writer import Writer
        import av
        import fractions
        ...
        # 获取首帧尺寸 (假设所有帧一致)
        # 注意: images中的数据通常是 (H, W, 3) 或者是 (C, H, W) 取决于来源
        # add_frame 中通常将 tensor 转为 numpy。
        # 如果是 (3, H, W) 需要转置为 (H, W, 3)
        img0 = images[0]
        if img0.ndim == 3 and img0.shape[0] == 3 and img0.shape[2] != 3:
            # Assume CHW, transpose to HWC for PyAV
            height, width = img0.shape[1], img0.shape[2]
        else:
        ...
        with open(output_path, "wb") as f:
            ...
            # Codec Initialisation (Golden Config)
            codec = None
            try:
                codec = av.CodecContext.create("hevc_nvenc", "w")
                codec.width = width
                codec.height = height
                codec.pix_fmt = "yuv420p"
                codec.gop_size = self.fps * 15
                codec.time_base = fractions.Fraction(1, self.fps)

                codec.options = {
                    "preset": "p7",
                    "rc": "vbr",
                    "cq": "36",  # CQ 36 (配合无损内存源图效果极佳)
                    "maxrate": "3M",  # 3M 上限
                    "bufsize": "6M",
                    "b:v": "0",
                    "bf": "0",  # 禁用B帧
                ...
                print(f"NVENC Failed: {e}, falling back to libx265")
                codec = av.CodecContext.create("libx265", "w")
                codec.width = width
                codec.height = height
                codec.pix_fmt = "yuv420p"
            ...
            for packet in codec.encode(None):
                if packet.pts is not None:
                    packet.dts = packet.pts
                log_time = int(len(images) * 1e9 / self.fps)
                writer.add_message(
                    channel_id=channel_id,
                    log_time=log_time,
                    data=bytes(packet),
    ...
    def _save_episode_mcap(self, input_dir, output_file):
        """合并input_dir中所有frame-*.mcap文件并进行H265硬编码 (优先尝试 hevc_nvenc)"""
        import av
        import fractions

        input_dir = Path(input_dir)
        mcap_files = sorted(
            input_dir.glob("frame-*.mcap"), key=lambda x: int(x.stem.split("-")[-1])
        )

        if not mcap_files:
            print(f"在 {input_dir} 中找不到 frame-*.mcap 文件")
            return
        ...
        with open(output_path, "wb") as out_f:
            ...
            channel_id = writer.register_channel(
                topic="camera_image",
                message_encoding="video/h265",  # 强制与下方的 hevc_nvenc 编码器保持一致
                schema_id=0,
            )
            ...
            for i, mcap_file in enumerate(mcap_files):
                with open(mcap_file, "rb") as in_f:
                    reader = make_reader(in_f)

                    for schema, channel, message in reader.iter_messages():
                        if channel.message_encoding not in [
                            "image/jpeg",
                            "image/jpg",
                            "image/png",
                        ]:
                        ...
                        try:
                            ...
                            if encoding not in decoders:
                                codec_name = (
                                    "png" if encoding == "image/png" else "mjpeg"
                                )
                                decoders[encoding] = av.CodecContext.create(
                                    codec_name, "r"
                                )

                            packet = av.Packet(message.data)
                            frames = decoders[encoding].decode(packet)
                            if not frames:
                                continue
                            frame = frames[0]
                        except Exception as e:
                        ...
                        if not is_initialized:
                            # 尝试初始化硬编码器 (hevc_nvenc)
                            try:
                                codec = av.CodecContext.create("hevc_nvenc", "w")
                                codec.width = width
                                codec.height = height
                                codec.pix_fmt = "yuv420p"
                                # codec.rate = self.fps  # 移除：这会干扰码率控制 (Rate Control)
                                codec.gop_size = self.fps * 4
                                codec.time_base = fractions.Fraction(1, self.fps)

                                # NVENC 抗噪优化配置
                                # 增加 AQ 和 Lookahead，让编码器把码率用在刀刃上，忽略随机噪点
                                codec.options = {
                                    "preset": "p4",
                                    "rc": "vbr",
                                    "cq": "26",  # 26 -> 28: 稍微牺牲一点高频细节来抹平底噪
                                    "maxrate": "5M",
                                    "bufsize": "10M",
                                    "b:v": "0",
                                    "bf": "3",
                                    "temporal-aq": "1",  # 时域自适应量化
                                    "spatial-aq": "1",  # 新增: 空域自适应量化 (降低平坦区域的噪点权重)
                                    "rc-lookahead": "32",  # 新增: 预读32帧，更好地区分真实运动和随机噪点
                                }
                                codec.open()
                                print(f"成功初始化 H265 硬编码器: hevc_nvenc")
                            except Exception as e:
                                print(f"无法初始化 hevc_nvenc 硬编码器: {e}，回退到 libx265 软编码")
                                codec = av.CodecContext.create("libx265", "w")
                                codec.width = width
                                codec.height = height
                                codec.pix_fmt = "yuv420p"
                                codec.rate = self.fps
                                codec.options = {"crf": "26", "preset": "fast"}
                                codec.open()

                            is_initialized = True

                        # 确保像素格式符合编码器要求 (yuv420p)
                        ...
                        last_publish_time = message.publish_time
            ...
            if codec is not None:
                for packet in codec.encode(None):
                    writer.add_message(
                        ...
                        data=bytes(packet),
                        publish_time=last_publish_time,
                    )
    ...
    def start_mcap_writer(self, num_processes: int = 0, num_threads: int = 4) -> None:
        if isinstance(self.image_writer, AsyncMcapWriter):
            logging.warning(
    ...
    def create(
        ...
        obj.revision = None
        obj.image_buffer = None
        obj.tolerance_s = tolerance_s
        obj.image_writer = None
        obj.streaming_encoders = {}

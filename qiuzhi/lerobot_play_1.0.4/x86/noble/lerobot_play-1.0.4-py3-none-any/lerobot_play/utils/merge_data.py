            ...
            try:
                ...
                if (
                    ...
                    if current_task_index in folder_task_mapping[old_folder]:
                        new_task_index = folder_task_mapping[old_folder][
                            current_task_index
                        ]
                        print(
                ...
                error_msg = (
                    f"处理 {source_path} 失败: {e} (Processing {source_path} failed: {e})"
                )
                print(error_msg)
                traceback.print_exc()
                failed_files.append(
                    ...
                    if file.endswith(".parquet") and f"episode_{old_index:06d}" in file:
                        try:
                            ...
                            if (
                                ...
                                if (
                                    current_task_index
                                    in folder_task_mapping[old_folder]
                                ):
                                    new_task_index = folder_task_mapping[old_folder][
                                        current_task_index
                                    ]
                            ...
                            found = True
                            print(
                                f"已处理并保存: {dest_path} (Processed and saved: {dest_path})"
                            )
                            break
                        except Exception as e:
    ...
    for folder in source_folders:
        try:
            ...
            for task in folder_tasks:
                ...
                # 保存此文件夹中旧索引到新索引的映射
                folder_task_mapping[folder][old_index] = task_desc_to_new_index[
                    task_desc
                ]
            ...
            for episode in episodes:
                ...
                if old_index in stats_map:
                    ...
                    if (
                        "stats" in stats and folder_dimensions[folder] < max_dim
                    ):  # 使用变量替代硬编码的18
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fps", type=int, default=20, help="Your datasets FPS (default: 20)"
    )

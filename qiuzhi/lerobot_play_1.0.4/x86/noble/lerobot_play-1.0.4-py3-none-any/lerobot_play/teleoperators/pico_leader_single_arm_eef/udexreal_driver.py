    ...
    def end_listening(self):
        if self.recv_thread and self.recv_thread.is_alive():
            ...
            if self.recv_thread.is_alive():
                print("Warning: recv_thread did not exit gracefully within timeout")
        else:
            print("recv_thread is not alive or does not exist")
        ...
        try:
            ...
            if len(role_list) > 0:
                ...
                for role_name in role_list:
                    ...
                    controller_data = sdk.get_vec_controller_data(
                        role_name
                    )  # Get controller data
                    ...
                    for i, header in enumerate(GloveDataHeaders):
                        print(
                            f"{header}: ({finger_data[i].x}, {finger_data[i].y}, {finger_data[i].z})"
                        )

...
from lerobot_play.teleoperators.quest3_leader.config_quest3_leader import (
    Quest3LeaderConfig,
)
from lerobot_play.teleoperators.quest3_leader import quest_webrtc
from .lpf import OnlineVariableStepLPF

# from airbot_kdl.arm_kdl_ops import ArmKdlNumerical
from mmk2_kdl_py import ArmKdlNumerical

logger = logging.getLogger(__name__)


class Quest3Leader(Teleoperator):
    config_class = Quest3LeaderConfig
    name = "quest3_leader"

    def __init__(self, config: Quest3LeaderConfig):
        ...
        # self.arm_kdl_v2 = ArmKdlNumerical(arm_type="play_v2", eef_type="G2")
        self.arm_kdl_v2 = ArmKdlNumerical(eef_type="G2")
        self.lpfs_left = [
        ...
        self.prev_result_left = [0.0] * 6
        self.prev_result_right = [0.0] * 6
    ...
    def get_smooth_ik_solution(
        self, target_pose, prev_solution=None, joint_weights=None
    ):
        """
        最简单实用的防翻腕方案
        """
        ...
        # 1. 获取所有解析解
        all_solutions = self.arm_kdl_v2.inverse_kinematics(target_pose)
        ...
        for sol in all_solutions:
            ...
            # 基础：关节变化越小越好
            delta = sol - prev_solution

            # 特别关注关节4和6（腕部关节）
            for i in range(6):
                if i == 3:  # 关节4和6
                    # 腕部关节：惩罚大的变化
                    if abs(sol[i]) > math.pi / 3:
                        score += 300
                    angle_diff = abs(delta[i])
                    if angle_diff > np.radians(math.pi * 3 / 5):  # 超过90度视为翻腕
                        score += angle_diff * 70  # 严重惩罚
                    else:
                        score += angle_diff * 5.0  # 一般惩罚
                if i == 5:  # 关节4和6
                    # 腕部关节：惩罚大的变化
                    angle_diff = abs(delta[i])
                    if angle_diff > np.radians(math.pi * 3 / 5):  # 超过90度视为翻腕
    ...
    def get_end_pose(self):
        joints = [0.0] * 6
        end_pose_mat = self.arm_kdl_v2.forward_kinematics(joints)

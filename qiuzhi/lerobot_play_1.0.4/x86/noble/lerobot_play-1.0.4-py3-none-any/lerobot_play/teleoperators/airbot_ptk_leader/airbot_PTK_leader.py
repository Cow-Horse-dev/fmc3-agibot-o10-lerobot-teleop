...
from lerobot_play.teleoperators.airbot_ptk_leader.config_PTK_leader import (
    AirbotPTKLeaderConfig,
)

logger = logging.getLogger(__name__)


class AirbotPTKLeader(Teleoperator):
    config_class = AirbotPTKLeaderConfig
    name = "airbot_PTK_leader"

    def __init__(self, config: AirbotPTKLeaderConfig):
    ...
    def connect(self) -> None:
        ...
        if not self.left_arm.init(self.io_context_left, self.left_arm_port, 250):
            raise RuntimeError("Failed to initialize left_arm")
        if not self.right_arm.init(self.io_context_right, self.right_arm_port, 250):
            raise RuntimeError("Failed to initialize right_arm")
    ...
    def get_action(self):
        ...
        action_dict = {
            ...
            "left_eef.pos": max(0.0, min(action[0][6] * 1.5, 0.072)),
            ...
            "right_eef.pos": max(0.0, min(action[1][6] * 1.5, 0.072)),
        }
        return action_dict
...
if __name__ == "__main__":
    ...
    teleop_config = AirbotPTKLeaderConfig(
        left_arm_port="can2",
        right_arm_port="can3",
        id="PTK_leader",
    )
    arm = AirbotPTKLeader(teleop_config)
    arm.connect()
